...
The pillar scores 830/1000 on agent completability (83%). Human involvement is concentrated at dataset sourcing, entity extraction thresholds, domain knowledge for Australian cadastral data, CRS edge case review, and approval of ADR-016 and the temporal field migration.
...
**Start Condition:** Phase 2 complete (M5 and M6 pass). ADR-016 drafted and accepted by Mikey. NSW Planning Portal API endpoint validated.
...
**Deliverables:**
- ADR-016: Temporal Trigger Architecture (accepted)
...
| Session | Objective | Estimated Tokens | Estimated Duration | Inputs Required | Output |
|---|---|---|---|---|---|
| Session 08 | ADR-016 drafting | 40k–60k | 1–2 hours | Dr. Voss M7 spec, Pillar 1 ADR-007 | ADR-016 draft flagged for Mikey approval |
| Session 09 | Permit adapter, resolver, state transition service (M7) | 120k–180k | 3–5 hours | Validated NSW Planning Portal API, ADR-016 accepted | Temporal trigger subsystem with test suite |
...
| Sprint 3 (Weeks 5–6) | Phase 2 → Phase 3 | Sessions 07–08: Multi-dataset ingestion, ADR-016 drafting | M6, ADR-016 draft | Three datasets available |
| Sprint 4 (Weeks 7–8) | Phase 3 | Sessions 09–10: Temporal trigger layer build, integration, acceptance | M7 | ADR-016 accepted, NSW API validated, migration approved |

**Note:** Sprint 4 may compress to 1 week if ADR-016 approval and NSW API validation happen promptly. The 5–6 week estimate assumes normal approval cadence.
...
| Central Coast NSW datasets sourced | External | Mikey | **Resolved** — programmatic access validated for all four sources |
...
|---|---|---|
| Pillar 3 — Rendering Interface | Populated cells with geometry and fidelity_coverage metadata to render | Before Pillar 3 can render anything real (Pillar 3 M2+) |
...
|---|---|---|---|
| ADR drafting session | 40k–60k | 1 session (ADR-016) | Lower token intensity, high reasoning load |
...
| M7 — Temporal Trigger Layer | Permit feed connected; cell state transitions live; bitemporal fields active | Sprint 4 | All 8 AC from Dr. Voss's specification pass (AC1–AC8). ADR-016 accepted. Migration approved and executed. Gosford DA fixture resolves correctly. | M6, ADR-016 accepted, NSW API validated, migration approved |
...
| 7 | ADR-016 approval delayed | Low | Medium — M7 blocked | Draft ADR early in Sprint 3. Flag to Mikey with clear approval deadline. | Mikey |
...
- Central Coast datasets sourced → Sprint 1 start (resolved — programmatic access validated)
- ADR-016 accepted → Milestone 7 start (resolved — accepted April 22, 2026)
- Mikey approval of temporal field migration → M7 completion (pending)
- NSW Planning Portal API validated → M7 build start (pending — Data Broker response)
...
| 3 | Review and accept ADR-016 (Temporal Trigger Architecture) | Sprint 3 | 30–60 minutes | M7 |
